...
class adminCog(commands.Cog):
    def __init__(self, bot):
        ...
        self.servers = None
        self.bot = bot
        #TODO: Make sure that RoleID for Support Members is properly programmed for the cogs. This would be used for
        # those who do not have the Administrator permission in any of their roles. But are support members
    ...
    #Issues Global DeviantCord Support Code to all guilds
    async def issue_gds_code(self, ctx, staffid:int, provided_guildid:int):
        if ctx.guild is None:
            return
        if self.guildid is None:
            return
        ...
        if provided_guildid == self.guildid:
            if ctx.message.channel.id == self.errite_channel_id:
                ...
                designated_staff = self.bot.get_user(staffid)
                if designated_staff is None:
                    await ctx.send("Invalid staff member id given")
                    return
