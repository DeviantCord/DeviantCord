...
def syncListeners(conn, task_cursor, source_cursor):
    """
    ...
    change_sql = """ UPDATE deviantcord.deviation_listeners
                    ...
                    last_ids = data.last_ids FROM (VALUES %s) AS data(dcuuid, last_update, last_ids, artist, folderid, serverid, channelid)
    ...
    change_all_sql = """ UPDATE deviantcord.deviation_listeners
                         SET dc_uuid = data.dcuuid, last_update = data.last_update, 
                        last_ids = data.last_ids FROM (VALUES %s) AS data(dcuuid, last_update, last_ids, artist,serverid, channelid, mature)
    ...
    task_get_sql = "select * from deviantcord.deviation_listeners "
            ...
            if not dc_uuid == obt_dcuuid:
                if hybrid:
                    ...
                    # If there are only deviations and not hybrid deviations
                        ...
                        hybrid_only_commits.append((obt_dcuuid, timestr, obt_hybrid_ids, artist, folderid, serverid, channel_id))
...
def addalltask(serverid:int, channelid:int, artistname, mature, conn):
    ...
    task_cursor.execute(sql,(serverid, artistname, "none", "all-folder", dcuuid, False, [], channelid, timestr, timestr,
